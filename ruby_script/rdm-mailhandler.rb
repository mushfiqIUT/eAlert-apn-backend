...
# == Usage
#
#    rdm-mailhandler [options] --url http://192.168.10.153/ --key CE9saVCBPsFJkGgK661f
#
# == Arguments
# 
...
## TEMP TEST
#     params = params.gsub('@localhost','XX')s
#     params = '/usr/local/bin/ruby /etc/postfix/rdm-mailhandler.rb --url=http://localhost --key=CE9saVCBPsFJkGgK661f --project=svn-redmine --no-permission-check --unknown-user=accept --category=bug --allow-override=project'
...
class RedmineMailHandler
  ...
  #attr_accessor :verbose, :issue_attributes, :allow_override, :unknown_user, :no_permission_check, :project, :url, :key
...
###### Test  ###################
	when '--project'
      ...
      when '--status', '--tracker', '--category', '--priority'
        self.issue_attributes[opt.gsub(%r{^\-\-}, '')] = arg.dup 
       when '--allow-override'
  ...
  def submit(email)	
	originator = /Delivered-To: act-elit-alert@localhost/
	parts = email.match(originator)

	if parts != nil		
		data = { 'notify' => 'true',
			'username' => 'elitalerts',
			'password' => '45xr61ElitAlerts'}

    uri = 'https://elitalerts.impulsebdltd.com/elitalerts/notify.php'
		...
		if response.code.to_i == 200
			...
			warn "Failed to send notification (#{response.code})."
			return 77
		end
	end
    ...
    #issue_attributes.each { |attr, value| data["issue[#{attr}]"] = value.gsub(%r{@localhost},'') }

    issue_attributes.each { |attr, value| data["issue[#{attr}]"] = if(attr == 'project') then value.gsub(%r{@localhost},'' ) else value end }
